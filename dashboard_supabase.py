...
try:
    ...
    _PLOTLY_STACK_ERROR = _e_plot
    print(f"numpy/pandas/plotly import failed: {_e_plot}", flush=True)
    np = pd = px = go = None  # type: ignore[assignment,misc]
...
""", unsafe_allow_html=True)
    ...
    print(f"Global CSS (st.markdown) failed: {_e_css}", flush=True)
...
def main():
    if _PLOTLY_STACK_ERROR is not None:
        st.title("Эстадель — Аналитика")
        st.error("Не удалось загрузить numpy / pandas / plotly. Проверьте `requirements.txt` (plotly, pandas) на Streamlit Cloud.")
        st.exception(_PLOTLY_STACK_ERROR)
        return
    if _DATA_IMPORT_ERROR is not None:
        st.title("Эстадель — Аналитика")
        st.error("Не удалось импортировать `dashboard_supabase_data.py` (зависимости или синтаксис).")
        st.exception(_DATA_IMPORT_ERROR)
        return
    st.title("Эстадель — Аналитика")
